...
2. Run the dedupe gate before coding: compare title, issue, changed files, traceback/function names, closed PR lineage, salvage PRs, forks, and remembered blockers.
3. Select one small candidate: prefer review or consolidation when an active PR already covers the area; open a new PR only for a narrow, low-risk, testable slice with no overlap.
...
6. Prepare public output: PR text must explain problem, solution, validation, risk, and related issue or credit; never include internal scoring, volume strategy, or private procedure.
