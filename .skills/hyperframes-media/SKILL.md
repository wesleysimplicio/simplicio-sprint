...
description: Preprocess assets for HyperFrames compositions — local TTS (Kokoro-82M), word-level transcription (Whisper), and background removal (u2net). Use when the user needs narration audio, captions, or a transparent-subject overlay. Each command downloads its own model on first run.
---
...
> Mirror of upstream `skills/hyperframes-media/SKILL.md`.
...
Use `--lang` only to override auto-detection. Valid codes: `en-us`, `en-gb`, `es`, `fr-fr`, `hi`, `it`, `pt-br`, `ja`, `zh`. Non-English phonemization requires `espeak-ng` system-wide (`brew install espeak-ng` / `apt-get install espeak-ng`).
...
Music with vocals: start at `medium` minimum; produced tracks often need manual SRT/VTT import. For caption-quality checks (mandatory after every transcription), cleaning JS, retry rules, and the OpenAI/Groq API import path, see upstream `references/transcript-guide.md`.
...
Chrome decodes VP9 alpha natively, so the `.webm` plugs into a composition like any other muted-autoplay video.
