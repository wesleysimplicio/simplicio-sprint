...
def test_parse_azure_sprint_url_supports_project_prefixed_taskboard_tail() -> None:
    parts = parse_azure_sprint_url(
        "https://dev.azure.com/DigitalProjects-Americas/ONS-16058-MANUTSIS-FORT/"
        "_sprints/taskboard/ONS-16058-MANUTSIS-FORT/Time_019/T019_Sprint_98"
    )
...
def test_parse_azure_sprint_url_supports_visualstudio_host() -> None:
    parts = parse_azure_sprint_url(
        "https://myorg.visualstudio.com/MyProject/_sprints/taskboard/MyTeam/Sprint 12"
    )

    assert parts.organization == "myorg"
    assert parts.project == "MyProject"
    assert parts.team == "MyTeam"
    assert parts.iteration_name == "Sprint 12"
    assert parts.iteration_path == "MyProject\\MyTeam\\Sprint 12"
